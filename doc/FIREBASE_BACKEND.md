...
- Public HTTP functions serving app data.
...
|---|---|---|
| `deadlineFoodBootstrap` | `GET`, `HEAD`, `OPTIONS` | Returns full bootstrap payload: meals, canonical constraints, app metadata. |
...
If the document does not exist, `getAppData()` seeds it from generated static data.

### `recipes/{recipeId}`

Canonical recipe store (issue #123). Holds the full app `Meal` content
...
- `deadlineFoodRecipes` — lists recipes; seeds the collection from the generated
  `appRecipes` on first read if empty.
...
- `scripts/export-firebase-app-data.ts`
...
Do not manually edit `functions/src/generated/appData.ts`; regenerate it.
